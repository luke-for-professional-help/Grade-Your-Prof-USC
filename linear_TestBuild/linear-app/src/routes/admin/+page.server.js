...
export async function load({ url }) {
	...
	try {
		...
		const [[{ totalUsers }]] = await pool.query(
			'SELECT COUNT(*) as totalUsers FROM user WHERE Status_ID = 2'
		);
		...
		const [[{ totalReqs }]] = await pool.query(
			'SELECT COUNT(*) as totalReqs FROM user WHERE Status_ID = 1'
		);
		...
		const [[{ totalBlack }]] = await pool.query(
			'SELECT COUNT(*) as totalBlack FROM user WHERE Status_ID = 3'
		);
...
export const actions = {
	moderateUser: async ({ request }) => {
		...
		await pool.query('UPDATE user SET Status_ID = ?, Ban_Time = NULL WHERE User_ID = ?', [
			statusId,
			userId
		]);
		return { success: true };
	...
	banUser: async ({ request }) => {
		...
		await pool.query(
			`UPDATE user SET Status_ID = 3, Ban_Time = DATE_ADD(NOW(), INTERVAL ? HOUR) WHERE User_ID = ?`,
			[banHours, userId]
		);
		return { success: true };
	},
	...
	updateRole: async ({ request }) => {
		...
		await pool.query('UPDATE user SET isModerator = ?, isAdmin = ? WHERE User_ID = ?', [
			isMod,
			isAdmin,
			userId
		]);
